...
- **Timestamps split into two categories, and every one of them must say
  ...
  - **Artifact tree "Apps" node — SUPERSEDED 2026-09-15, kept below for
    the field-derivation history, not the architecture.** This whole
    entry describes a hardcoded GUI tree node that no longer exists — per
    direct user request, it was converted into two normal, manually-run
    parser scripts (`artifacts/ios/app_report.py`,
    `artifacts/android/app_report.py`), run from the "Run Artifact
    Parsers" dialog like every other report, not auto-populated at case
    load. See the "App Report parser scripts (device_wide API)" entry
    further below for the current architecture, and TODO.md item 26 for
    the full conversion writeup. Every WHERE-EACH-FIELD-COMES-FROM/
    WHY-THIS-DESIGN decision below is still accurate and still governs
    `app_intelligence.scan_apps()`'s real behavior (unchanged by the
    move) — only the "there's a special hardcoded Apps tree node" framing
    is now wrong; `_ART_APPS`/`_art_show_apps`/`_populate_apps_table`/
    `AppIntelligenceWorker`/the Category+Date filter UI named throughout
    this entry are all deleted code, not a current API to call into.
  - **Artifact tree "Apps" node [historical]** (`artifact_viewer.py`, added
  ...
  - **Parser version tracking** (`app/parser_versions.py`, added
    ...
    This only ever detects THIS script's own content changing — see the
    "Coverage-fingerprint staleness banner" paragraph in the "App Report
    parser scripts (device_wide API)" entry below for the separate
    mechanism (`get_coverage_fingerprint`, a `device_wide`-only
    extension) that catches a DIFFERENT parser being added/updated
    instead.
...
- **Real beach-ball on case load, found and fixed by direct profiling —
  ...
  loading the massive tree into the gui." Investigated rather than
  ...
  `_populate_tree_children_batched`) — profiling a real reopen of the
  830,298-entry IOS17 JoshHickman archive with cProfile found the actual
  cause instead: `on_metadata_ready` (the main-thread slot that fires the
  instant metadata arrives) was blocking for 4.1–4.4 seconds, 90% of it
  one call — `zip_cd_cache.load()` re-parsing the ENTIRE central
  directory through `zipfile.ZipFile()` from scratch, synchronously, on
  the GUI thread.

  A direct user follow-up question — "do we not already have the data as
  part of the cache we have created using the sidecar" — surfaced the
  real, systemic gap: the `.zcd` sidecar already avoids re-fetching the
  central directory over the NETWORK, but `zip_cd_cache.load()` had ZERO
  memoization of its own PARSED result — every one of 17+ independent
  ...
  every nested-archive extraction, every single-file scan, ...) re-paid
  the same ~1.6–3.9s parse from scratch, every single call, for the life
  of a session.

  Two fixes: (1) `zip_cd_cache.load()` now memoizes its own parsed
  `ZipInfo` list in-memory, keyed by the real `.zcd` file path (never
  bare `zip_path` — the same archive can legitimately be open under two
  ...
  invalidated by the `.zcd` file's own `(mtime, size)`. Verified directly:
  1st call 1.6s, 2nd/3rd calls 0.0000s (same object returned, no
  needless copy); after touching the real file's mtime, correctly
  re-parsed a fresh object rather than serving stale data. (2)
  `ffs-explorer.py`'s `on_metadata_ready` no longer builds
  `self._zip_handle` synchronously — submitted to the existing `_BG_POOL`
  thread pool via a new module-level `_build_cached_zip_view` helper,
  reusing `_get_zip_handle()`'s own ALREADY-EXISTING
  `self._zip_open_future` lazy-fallback mechanism (a dormant code path
  left over from before an earlier session's zipfile-elimination sweep
  removed its previous raw-zipfile use — confirmed nothing else in the
  codebase reads `self._zip_handle` directly, bypassing that getter,
  before reusing it for this).

  Verified end-to-end against real data: `on_metadata_ready` itself
  dropped from 4.1–4.4s to a consistent ~1.6–1.9s across three
  fresh-process runs; `_get_zip_handle()` called after load correctly
  returned a working handle with the background future already resolved.
  Sub-step timing then isolated exactly what's left:
  `_art_select_and_show_apps` (the Apps-tree-node default-view
  population, a supposedly-fast "cache hit" path per its own existing
  design — see the `app_intelligence.py` entry above) now accounts for
  ~90% of the remaining time (0.6–1.0s) — a separate, smaller, not-yet-
...
- **App Report parser scripts (device_wide API)** (2026-09-15) — what
  ...
  above) is now `artifacts/ios/app_report.py` and
  `artifacts/android/app_report.py`, two normal parser scripts run
  manually from the "Run Artifact Parsers" dialog, per direct user
  request. Both are thin wrappers over `app_intelligence.scan_apps()` —
  the underlying computation, and every design decision documented in
  the SUPERSEDED entry above about what each field means and how it's
  derived, is completely unchanged; only how the GUI reaches it changed.

  **New `device_wide` parser API** (`app/artifact_runner.py`'s own
  module docstring has the terse reference; this is the narrative
  version). A parser declares `device_wide = True` instead of
  `app_path`/`app_group`/`files` — there's no single app to scope to,
  `scan_apps()` walks the whole device. `run_artifact()` skips the
  normal files/optional_files resolution entirely and instead builds
  `paths['_case_context']`, a real `mcp_server.CaseContext` — the SAME
  object shape `scan_apps()` already expected — constructed via a new
  `_build_case_context()` helper.

  The context is loaded fresh from the case's own persisted
  load-snapshot (`ffs_metadata.load_snapshot_from_case`, extracting the
  exact same read `ZipMetadataWorker._try_load_from_snapshot` already
  does for an instant re-open) plus `folder_sizes`/`guid_to_bundle` from
  `casecache.db` — deliberately never the live GUI window object, so a
  device-wide parser stays exactly as headless/testable as every other
  parser script (this was a real design fork: `mcp_server.CaseContext`
  itself turned out to be a LIVE bridge from the GUI's in-memory state,
  not something reusable as-is from a headless script — confirmed by
  reading its own dataclass fields, a real correction to an initial
  hope). `raw_content_enabled` is always `True` (a direct,
  examiner-triggered parser run against the examiner's own already-open
  case — same reasoning the old GUI Apps view already used, no
  AI/MCP-consent boundary to protect here).

  Two real, non-obvious bugs were caught by direct testing before this
  shipped: (1) `_make_zip_byte_reader`'s reserved `_read_zip_bytes` key
  (already exposed to every multi-file parser) takes a PHYSICAL zip
  entry name, but `CaseContext.read_bytes` is called with a UI_PATH that
  the callee resolves itself — confirmed via `scan_apps()`'s own real
  usage and the old GUI construction's own `hex_viewer._read_zip_bytes`
  pattern. Passing the physical-path reader straight through as
  `read_bytes` would have silently failed every single read (a ui_path
  never matches a real physical entry name for a GUID-containerized iOS
  app) — fixed with a wrapper that resolves first. (2) The very first
  real test against the IOS17 JoshHickman archive returned 0 apps —
  root cause: `scan_apps()` resolves every GUID-named container via
  `container_bundle_id(child, guid_map)`, and an empty/wrong
  `guid_to_bundle` silently resolves every container to nothing, not an
  error. `run_artifact()` already receives a `guid_to_bundle` parameter
  from its own caller, but `_build_case_context` doesn't trust it — it
  loads `guid_to_bundle` fresh from `casecache.db`
  (`db_utils.load_guid_bundle_map`) itself, making the whole context
  construction self-sufficient from `case_dir` alone rather than
  dependent on every future `run_artifact()` caller threading this
  through correctly.

  `app_intelligence.py` gained two new Qt-free functions,
  `build_app_registry_lookup`/`flatten_row`, ported behavior-for-behavior
  from what used to be `artifact_viewer.py`'s own GUI-only
  `_build_app_registry_lookup`/`_flatten_app_intelligence_row` — same
  logic, just returning a normal snake_case row dict (`display_name`,
  `app_id`, `shared_data_folder`, `total_bytes`, ...) instead of the old
  GUI's positional tuple + separate Title-Case header list, matching
  every other parser's own row-dict convention. The iOS parser's real,
  useful documentation about what its four timestamp columns actually
  measure and their limits (fabricated far-future mtimes from
  third-party disk-cache libraries, etc.) — previously a special
  hand-authored "Application Report Notes" tree page reachable only from
  the old Apps node — moved into the parser's own `description`, which
  every report already surfaces via the standard "Report Notes/Warning"
  page; no special-casing needed for that content to keep existing.

  **New `byte_fields` declarative convention** (`_art_show_report`,
  matching `timestamp_fields`/`media_fields`'s own shape exactly) —
  generalizes what used to be a one-off manual
  `set_byte_columns(["Total Size"])` call hardcoded to the old Apps
  table's own list-mode display into something any DB-backed report can
  declare: a raw byte-count column stays numerically sortable in
  storage, MB-formatted only at display time.

  **Tree restructuring**: the former fixed "Apps" parent — which every
  standalone report AND every app-grouped report used to nest inside —
  is gone; `_refresh_artifact_tab`'s build loop now appends both
  directly to the tree root. Per-app GROUPING itself
  (`app_group_label` — e.g. Chrome's 3 sub-reports sharing one "Chrome"
  node) is completely unchanged, confirmed by direct user instruction to
  preserve it and verified against a real case afterward.
  `ffs-explorer.py`'s `on_metadata_ready` no longer force-selects
  anything at case load — confirmed via direct research that the "Jump
  to this row in the report" feature's own tree search already used a
  generic depth-first role search with no position-0 dependency (so it
  needed no change), and that MCP's `list_apps` tool independently
  builds its own `CaseContext` and shares only the `casecache.db` cache
  table as a pure cache, with zero dependency on this GUI tree (also
  needed no change).

  A real, acknowledged loss, openly flagged rather than silently
  dropped: the entire Category-checklist + Date-range filter UI the old
  Apps table had (added 2026-08-26, per an earlier direct request, for
  triaging a ~1000-row table) was Apps-specific special UI, not a
  generic report feature, and was removed along with everything else —
  a normal report only has the standard free-text Filter + Columns
  dialog. Picking that convenience back up as a new declarative
  convention, if missed, would be a separate future request.

  **Verified end-to-end against real data at every layer**: `scan_apps()`
  + flattening tested directly against two real cases (1,096 real apps
  on IOS17 JoshHickman including correct WhatsApp/Instagram/TikTok rows;
  197 real apps on a real Android case including a real 1.86GB WeChat
  container); then the full pipeline driving the actual `FastZipBrowser`
  in-process — a freshly loaded case shows no "Apps" node anywhere and
  stays on the blank placeholder; running the new parser via the real
  `ArtifactRunnerWorker` (the exact class the "Run Artifact Parsers"
  dialog uses) wrote 1,096 real rows to `caseresults.db`; the refreshed
  tree correctly shows "App Report" as a normal standalone top-level
  item; `_art_show_report('app_report')` displayed all rows via the
  standard DB-mode path with `byte_fields` MB-formatting working
  correctly on real data. See TODO.md item 26 for the full writeup.

  **A fixed snapshot, same as every other artifact report — never
  auto-refreshed, including by an AI client reading it** (per direct
  user instruction, 2026-09-15, after confirming the MCP `query_artifact`
  tool can already read this report with zero new plumbing — it's just a
  normal `artifact_<script_name>` table, so `get_case_overview` and
  `query_artifact(name="app_report")` both already see it). Unlike the
  MCP `list_apps` tool (always a live `scan_apps()` call), `app_report`'s
  own rows are frozen at whatever moment an examiner last ran it from the
  "Run Artifact Parsers" dialog — re-running it is the only way to pick
  up a newly added/updated parser script's effect on `has_parser`/
  `score`/`category` (those are derived from which parsers exist AT SCAN
  TIME). This does NOT mean the underlying app list itself can go stale
  the way a live device's app list would — the archive is an immutable
  forensic acquisition, so the actual set of apps on it never changes —
  only this project's own parser COVERAGE of those apps can improve
  after the fact, which a stale snapshot won't reflect until re-run.
  Both parser scripts' own `description` states this directly, so the
  caveat travels with the report itself for any future reader — human or
  AI — not just this conversation.

  **Coverage-fingerprint staleness banner, 2026-09-15** (direct
  follow-up: "when a parser is updated that information is at the top of
  the parser and it can be rerun — can we also have that for the app
  report, i.e. if a new artifact script is added the [report] can see
  that and it ask the user if they want to rerun"). The existing "newer
  parser version available" banner (see "Parser version tracking"
  above) only ever detects a script's OWN content changing — it can't notice a
  DIFFERENT script being added/updated, exactly the case that makes
  `app_report`'s own `has_parser`/`score`/`category` stale. New
  `parser_versions.get_coverage_fingerprint(platform)` — a blake2b hash
  over every known parser's own version for that platform — changes
  whenever any parser's content changes, one is added, or one is
  removed. `run_log` gained a `coverage_fingerprint INTEGER` column
  (same migration pattern as `parser_version`), recorded by
  `ArtifactRunnerWorker.run()` ONLY for a `device_wide` module (every
  other parser's stays `NULL`). `_update_art_version_banner`
  (`artifact_viewer.py`) checks the existing own-version signal first,
  falling through — only for a `device_wide` module with a recorded
  fingerprint — to a coverage-fingerprint comparison with its own banner
  wording ("A parser script has been added or updated since this report
  last ran..."), reusing the same banner widget/Update button. Verified
  against the real IOS17 JoshHickman case DB by reproducing the exact
  decision logic against real inserted `run_log` rows (stale-fingerprint
  → banner; matching-fingerprint → no banner; non-`device_wide` parser
  with `NULL` fingerprint → no banner, zero effect on ordinary parsers) —
  test rows removed afterward. See TODO.md item 27 for the full writeup,
  including the one self-caught bug (`int.from_bytes(..., signed=True)`
