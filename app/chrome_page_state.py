"""chrome_page_state.py — vendored Chromium Blink PageState parser.

Origin : https://github.com/RyanDFIR/hindsight (pyhindsight/lib/page_state.py),
         from the local checkout at /Users/klastveita/script/hindsight-main,
         pyhindsight version 2026.06 (Hindsight's own `__version__`).
Author : Ryan Benson (Hindsight's maintainer/author, per hindsight-main's
         own pyproject.toml `authors` field and README).
License: Apache License 2.0 — Hindsight's own repo-level LICENSE.md is
         Apache-2.0; this individual file carries no separate per-file
         header in the original, so the repo-level license is what applies
         and is the one being honored here. Full license text:
         http://www.apache.org/licenses/LICENSE-2.0 (not reproduced in
         full inline, unlike the MIT-licensed CCL Forensics files this
         project also vendors, since Apache-2.0 is long; the original
         repository's LICENSE.md carries the complete text).

Unmodified — copied verbatim, no code changes (stdlib only:
dataclasses/io/logging/struct/typing). Added 2026-09-05 alongside the
vendored `ccl_chromium_pickle.py`/`ccl_chromium_snss.py` (MIT, CCL
Forensics, a DIFFERENT vendored dependency — see those files' own headers)
for `artifacts/android/chrome_sessions.py` and `artifacts/android/
chrome_app_tabs.py`: `parse_page_state(raw)` decodes the `page_state_raw`
blob every Chromium `NavigationEntry`/`SerializedNavigationEntry` carries
(both the desktop-style SNSS Session/Tab-restore format the ccl_chromium_*
files read, AND the native WebContentsState blob embedded inside Android
Chrome's own app_tabs/<id>/tab<N> TabState files — confirmed by direct
reverse-engineering against this project's own real Android 14 JoshHickman
data that both embed the identical per-entry payload shape) — real forensic
content beyond the bare url/title/timestamp already decoded by
NavigationEntry.from_pickle: referrer, scroll position, POST body/file-
upload details, embedded iframe (child frame) state, and any filled-in
form field NAMES (`FormElement.name`/`type` — deliberately NOT necessarily
every submitted VALUE; see this file's own PickleReader/MojoReader classes
for exactly what is and isn't captured). Handles both the legacy Pickle
wire format (PageState versions 11-25) and the newer Mojo-serialized wire
format (versions 26-33) a real Chrome build can write, entirely
independently of which container (SNSS vs Android TabState) the entry
came from — the PageState version number alone decides which reader this
file uses, not the caller.

The remainder of this file, starting with its own original module
docstring below, is reproduced completely unmodified from the original.
"""
